...
if __name__ == '__main__':
    ...
    out_dir = expanduser('~/o2c-thpt/rs/')
    output_file_name = out_dir + '_'.join(['curl',socket.gethostname(),sitename,url.split(':')[0] if proxy_port == '0' else 'ss',start]) +'.txt'
    ...
    while True:
        ...
        try:
            ...
            if p.poll() == None:
                ...
                if len(out_lines) > 2:                    
                    speed = [filter(None,l.split(' '))[6] for l in out_lines if l.startswith('100 ') and len(filter(None,l.split(' '))) > 6][0]#use avg speed
                    print speed
